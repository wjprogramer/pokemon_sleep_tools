part of 'pokemon_box_page.dart';

class _PokemonBoxLogic extends State<PokemonBoxPage> {
  ...
  // Sort properties
  late PokemonSortOptions _sortOptions;

  @override
  void initState() {
    super.initState();
    _searchOptions = _args.initialSearchOptions ?? PokemonSearchOptions();
    _sortOptions = PokemonSortOptions();
  ...
  Widget _buildBottomNavigationBarContent(List<PokemonProfile> profiles) {
    return BottomBarWithActions(
      ...
      isSearchOn: _searchOptions.isEmptyOptions() ? false : true,
      onSort: () async {
        final sortOptions = await showPokemonSortDialog(
          context,
          focusBasicProfile: false,
          initialSortOptions: _sortOptions,
        );
        if (sortOptions == null) {
          return;
        }
        ...
        _resultProfiles = sortOptions.sortProfiles(_resultProfiles);
        setState(() { });
      },
      onAscendingChanged: (v) async {
        _sortOptions.setAscending(v);
        _resultProfiles = _sortOptions.sortProfiles(_resultProfiles);
        setState(() { });
      },
      isAscending: _sortOptions.isAscending,
      sortText: _sortOptions.sortOptions.firstOrNull?.nameI18nKey.xTr,
      suffixActions: [
